...
module.exports.process_msg = function(ws, data){
	if(data.v === 1){
		if(data.type == 'create'){
			...
			if(data.cpf && data.nome && data.curso && data.user){
				chaincode.invoke.IFNMG([data.cpf, data.nome, data.curso, data.user], cb_invoked);				//create a new marble
			}
		}
			...
			if(data.cpf && data.user){
				chaincode.invoke.set_user([data.cpf, data.user]);
			}
		}
		else if(data.type == 'remove'){
			console.log('removing msg');
			if(data.cpf){
				chaincode.invoke.delete([data.cpf]);
			}
		}
